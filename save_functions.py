...
def save_multi(file_name, q):
    if q.empty():
        ...
        with open(file_name, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(['Sample'] + ['Channel 1 [V]'] + ['Channel 2 [V]'] + ['Channel 3 [V]']+ ['Channel 4 [V]'])
            ...
            for i in range(len(data[0])):
                csvwriter.writerow([i, data[0][i], data[1][i], data[2][i], data[3][i]])   
